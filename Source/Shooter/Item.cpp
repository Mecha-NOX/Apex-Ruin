...
void AItem::ItemInterp(float DeltaTime)
{
	...
	if (Character && ItemZCurve)
	{
		...
		// Vector from Item to Camera Interp Locationm, X and Y are zeroed out
...
void AItem::OnConstruction(const FTransform& Transform)
{
	...
	if (RarityTableObject)
	{
		...
		if (RarityRow)
		{
			...
			DamageModifier = RarityRow->DamageModifier;
			if (GetItemMesh())
			{
...
void AItem::UpdatePulse()
{
	...
	if (DynamicMaterialInstance)
	{
		...
		DynamicMaterialInstance->SetScalarParameterValue(TEXT("GlowAmount"), CurveValue.Y * FresnelExponent);
		DynamicMaterialInstance->SetScalarParameterValue(TEXT("GlowAmount"), CurveValue.Z * FresnelReflectFraction);
	}
}
