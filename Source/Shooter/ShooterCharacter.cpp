...
void AShooterCharacter::FireWeapon()
{
	...
	if (WeaponHasAmmo())
	{
		...
		SendBullet();
		PlayGunFireMontage();
		// Subract 1 from the Weapon's Ammo
...
void AShooterCharacter::CalculateCrosshairSpread(float DeltaTime)
{
	...
	if (bFiringBullet)
	{
		// Spread crosshairs when shooting very quickly
		CrosshairShootingFactor = FMath::FInterpTo(CrosshairShootingFactor, 0.3f, DeltaTime, 60.f);
	}
	else
	{
		// Shrink crosshairs when shooting stops very quickly
...
bool AShooterCharacter::TraceUnderCrosshairs(FHitResult& OutHitResult, FVector& OuthitLocation)
{
	...
	// Get World position and directions of crosshair
	...
	if (bShouldTraceForItems)
	{
		...
		if (ItemTraceResult.bBlockingHit)
		{
			...
			if (TraceHitWeapon)
			{
				...
				{
					// Not currently highlighting a slot; highlighting one
...
void AShooterCharacter::InterpCapsuleHalfHeight(float DeltaTime)
{
	...
	// -ve value if crouching; +ve value if standing
